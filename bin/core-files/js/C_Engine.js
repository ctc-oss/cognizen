...
var pageType_arr = ["textOnly", "graphicOnly", "top", "left", "right", "bottom", "sidebar", "tabsOnly", "tabsLeft", "revealRight", "revealBottom", "revealTop", "revealLeft", "flashcardText", "flashcardMedia", "multipleChoice", "multipleSelect", "multipleChoiceImageTop", "multipleChoiceImageLeft", "multipleChoiceImageRight", "multipleSelectImageTop", "matching", "matchingDrag", "unity", "unityOnly", "tabbedContentMedia"];
...
function addIndex(){
	...
	for(var i = 0; i < totalPages; i++){
		...
		//$(data).find("page").eq(currentPage).attr("id")
		$('#indexContent').append('<li class="menuItem" id="'+thisID+'"tag="'+i+'" myID="'+$(data).find("page").eq(i).attr("id")+'"><div id="commentSpot"></div><a href="#">'+ (i+1) + '. ' + $(data).find("page").eq(i).find('title').text() + '</a></li>');
		indexItem_arr.push("#" + thisID);
	}
	$('#indexContent').append('</ul>');
	$('#indexContent').menu({
		select: function(event, ui){
			ui.item.addClass('indexMenuVisited');
			ui.item.toggleClass('ui-state-disabled').siblings().removeClass('ui-state-disabled');
			
			if(indexState == true){
				currentPage = parseInt(ui.item.attr("tag"));
	...
	//Allow for drag and drop page order editing within the index.
	if(mode == "edit"){
		
		$("#indexContent").sortable({
			stop: function(event, ui){
				//Set variable to hold the old node position
