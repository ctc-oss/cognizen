...
function C_MultipleChoiceFancy(_type) {
	...
	var buildTemplate = function() {
		...
		for(var j = 0; j < order_arr.length; j++){
			...
			var msg = '<div class="option" id="' + myOption + '">';
				msg += '<input id="' + myOption + 'Check"';
				if(isMulti){
					msg += ' type="checkbox"';
				}else{
					msg += ' type="radio"';
				}
				msg += ' name=' + type + '" class="radio" value="' + myNode.attr("correct")+ '" />';
				msg += '<label id="label" class="fixFunkyIssue" for="'+ myOption +'Check">'+ myLabel + '. ' +myNode.find("content").text() +'</label>';
				msg += '</div>';
